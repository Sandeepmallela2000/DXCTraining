...
public class QueueInter {
	...
	public static List<Deque<Patient>> splitQueue(Deque<Patient> patientsQueue) {
		...
		Deque<Patient> elderlyQueue = new ArrayDeque<>();
	    Deque<Patient> otherQueue = new ArrayDeque<>();
	    
	    for (Patient patient : patientsQueue) {
	        if (patient.getAge() >= 60) {
	            elderlyQueue.add(patient);
	        } else {
	            otherQueue.add(patient);
	        }
	    }
	    
	    List<Deque<Patient>> result = new ArrayList<>();
	    result.add(elderlyQueue);
	    result.add(otherQueue);
	    return result;
	}
